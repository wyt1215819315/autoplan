...
public class BiliHelpUtil {
    ...
    public static Map<String, Object> getCoinBalance(BiliWebUtil biliWebUtil) throws Exception {
        ...
        JSONObject responseJson = biliWebUtil.doGet(URLConstant.BILI_GET_COIN_BALANCE);
        int responseCode = responseJson.getInteger("code");
    ...
    public static Map<String, Object> coinAdd(String bvid, int multiply, Boolean selectLike, BiliWebUtil biliWebUtil, String biliJct) throws Exception {
        ...
        if (!(boolean) coinAddedMap.get("data")) {
            ...
            JSONObject jsonObject = biliWebUtil.doPost(URLConstant.BILI_COIN_ADD, params, headers);
            if (jsonObject.getInteger("code") == 0) {
                msg += String.format("为 %s投币成功", videoTitle);
                map.put("data", true);
            } else {
                msg += "投币失败" + jsonObject.getString("message");
                map.put("data", false);
            }
        } else {
    ...
    public static Map<String, Object> isCoinAdded(String bvid, BiliWebUtil biliWebUtil) throws Exception {
        ...
        JSONObject result = biliWebUtil.doGet(URLConstant.BILI_IS_COIN + urlParam);

        int multiply = result.getJSONObject("data").getInteger("multiply");
        if (multiply > 0) {
    ...
    public static String getVipPrivilege(int type, String biliJct, BiliWebUtil biliWebUtil) throws Exception {
        ...
        params.put("csrf", biliJct);
        JSONObject jsonObject = biliWebUtil.doPost(URLConstant.BILI_VIP_PRIVILEGE_RECEIVE, params);
        int responseCode = jsonObject.getInteger("code");
        if (responseCode == 0) {
            if (type == 1) {
            ...
            throw new Exception("领取年度大会员每月赠送的B币券/大会员福利失败，原因: " + jsonObject.getString("message"));
        }
        return "";
    }
    ...
    public static String getVideoTitle(String bvid, BiliWebUtil biliWebUtil) throws Exception {
        ...
        JSONObject jsonObject = biliWebUtil.doGet(URLConstant.BILI_VIDEO_VIEW + urlParameter);

        if (jsonObject.getInteger("code") == 0) {
            title = jsonObject.getJSONObject("data").getJSONObject("owner").getString("name") + ": ";
    ...
    public static String chargeComments(String token, BiliWebUtil biliWebUtil, String biliJct) throws Exception {
        ...
        JSONObject jsonObject = biliWebUtil.doPost(URLConstant.BILI_CHARGE_COMMENT, postMap);
    ...
    public static int expConfirm(BiliWebUtil biliWebUtil) throws Exception {
        JSONObject resultJson = biliWebUtil.doGet(URLConstant.BILI_NEED_COIN_NEW);
        int getCoinExp = resultJson.getInteger("data");
    ...
    public static JSONObject getDailyTaskStatus(BiliWebUtil biliWebUtil) throws Exception {
        JSONObject jsonObject = biliWebUtil.doGet(URLConstant.BILI_REWARD);
        int responseCode = jsonObject.getInteger("code");
        if (responseCode == 0) {
            return jsonObject.getJSONObject("data");
        } else {
            logger.error(jsonObject.getString("message"));
            return biliWebUtil.doGet(URLConstant.BILI_REWARD);
        }
    }
